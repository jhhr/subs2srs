      ...
      foreach (List<InfoCombined> combArray in workerVars.CombinedAll)
      {
        ...
        Parallel.ForEach(workItems, parallelOptions, (item, state) =>
        {
          ...
          if (!File.Exists(outName))
          {
            ...
            try
            {
              // Grouped card: the range list shared with the video clip and the animated
              // snapshot (kept parts only, omitted lines cut away, gaps shortened when gap
              // removal is on). Several ranges always re-encode (the select filter needs
              // decoded audio); a single range takes the plain path below.
