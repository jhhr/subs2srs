...
- Answers are cached per episode (keyed by the exact lines the model saw, the limits, the model, the chunking and the prompt version), so Preview → Go and repeated runs cost nothing. Requests are retried with exponential backoff on rate limits and server errors, honouring `Retry-After`; a failed chunk falls back to the rules; malformed answers (overlapping or out-of-range groups) are repaired and logged, and the length limit is enforced on every proposal.
