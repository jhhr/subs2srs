...
- Preferences gain an **AI** section: an API key per provider (stored in `preferences.json`; the `ANTHROPIC_API_KEY`, `OPENAI_API_KEY` and `GEMINI_API_KEY` environment variables override them; keys are never written to the log), *AI Max Concurrent Requests* (0 = auto), *AI Max Retries*, *AI Max Retry Wait Seconds*, *AI Request Timeout Seconds*, the cache directory and *AI Grouping On Go*. (The per-provider requests-per-minute and concurrency preferences of earlier development builds are gone; leftover keys in `preferences.json` are ignored.)
