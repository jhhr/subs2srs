...
| `subs2srs.Tests` | `SubsRetimerLauncherTests.cs` | xUnit (+ env-gated) | The `subsretimer` launcher without the tool: argument list (`--print-output`, `--auto`, `--output`, encodings, `--` before the paths, utf-8 fallback), result parsing (last stdout line as the saved path, exit 0 without output and exit 2 as nothing saved, other codes as failure), a missing executable as a failure not an exception; with `SUBSRETIMER_EXE` set, two real runs on generated SRT files (saved path and segment summary, refuse-to-overwrite, missing input) |
