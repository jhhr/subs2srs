...
| `subs2srs.Tests` | `ClaudeCliTests.cs` | xUnit | The `terminal-` helpers without starting a process: prefix and model mapping, Claude-only model check, the child environment (no API key, no thinking, no telemetry), finding the executable (configured path, `.exe`, npm shim → native binary, shim alone → none), the `claude --help` flag probe (cached per executable, nothing optional assumed when it fails), the argument list (flags of the probed build only, schema round trip, long system prompt moved into the prompt or into a file, never `--bare`), the classify table (structured output, text only, upstream 429/529/5xx retried, unknown model and max-structured-output-retries terminal, usage-limit wording, non-JSON output) and the last-JSON-object rule |
...
| `subs2srs.Tests` | `ClaudeCliLiveTests.cs` | xUnit (live, skipped) | One grouping through the real `claude` process and a check of this build’s flags; runs only with `SUBS2SRS_AI_CLI_LIVE=1` and Claude Code signed in. Records the arguments, exit code and raw output |
